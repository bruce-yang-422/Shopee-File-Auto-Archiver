...
支援的檔案類型：
    1. 訂單檔案 (.xlsx)
       ...
       - 常見檔名格式：店家識別_店鋪ID_報表類型_YYYYMMDD_...
       - 範例：萌寵要當家_SH0001_訂單_20240115.xlsx
       - 店家識別規則：優先使用第 2 段（店鋪 ID）對應 SHOP_MAPPING，若失敗再用第 1 段（店名）
       - 日期規則：從檔名抓取所有 20xxxxxx（8 碼日期），取最後一個日期的年份與月份分類
       - 分類至：01_Order_Data/{店家資料夾}/{年份}/{月份}/
    
    2. 廣告檔案 (.csv)
       - 識別條件：副檔名為 .csv，且檔名包含 "蝦皮廣告"
       - 檔名格式：{店家識別}-蝦皮廣告_YYYY_MM_DD.csv 或 {店家識別}_蝦皮廣告_YYYY_MM_DD.csv
       - 範例：萌寵要當家-蝦皮廣告_2024_01_15.csv、SH0001_蝦皮廣告_2024_01_15.csv
       - 店家識別規則：先用「蝦皮廣告」前字串比對 SHOP_MAPPING；若前字串含 "_"，會再切段逐一比對
       - 日期規則：需符合 YYYY_MM_DD，使用 YYYY 與 MM 進行分類
...
SHOP_MAPPING = {
    ...
    "SH0029": "SH0029_毛孩福利倉", "毛孩福利倉": "SH0029_毛孩福利倉",
}
